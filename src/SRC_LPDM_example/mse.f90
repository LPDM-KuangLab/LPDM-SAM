...
module mse
  ...
  use grid, only: z, zi, dx, dy, dz, adz, dt, dtn, day0, RUN3D, &
       ...
       nchunk_x_gl, nchunk_y_gl, nsaveMSE, na
...
subroutine compute_and_increment_MSE_tendency(which_process)
  ...
  sli_before(:,:,:) = t(1:nx,1:ny,1:nzm) - sli_before(:,:,:)
  fmse_before(:,:,:) = t(1:nx,1:ny,1:nzm) - fmse_before(:,:,:)
  qtot_before(:,:,:) = - qtot_before(:,:,:)
...
subroutine writeMSE()
  ...
  nfields3D=50 ! many more coming with budget terms...
...
!!$    end do
    ...
    if(RUN3D) then
      ...
      filename='./OUT_3D/'//trim(case)//'_MSE_'//trim(caseid)//'_'// &
           TRIM(rankchar)//'_'//TRIM(timechar)//filetype
      open(46,file=filename,status='unknown',form='unformatted')
    else
      filetype = '.bin2D'
      filename='./OUT_3D/'//trim(case)//'_MSE_'//trim(caseid)//'_'// &
           TRIM(rankchar)//filetype
  ...
  if(nfields3D.ne.nfields1) then
    if(masterproc) print*,'write_fields3D error: expected nfields = ', nfields2D, ', actual nfields = ', nfields1
    call task_abort()
  end if
  if(masterproc) then
    close (46)
