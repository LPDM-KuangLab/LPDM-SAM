...
!BG add accumulation of microphysical process rates here [in main loop as rates are 1dim or in iice function]
 if (do_accumulate_micro_proc_rates) then
    ...
    !idx = idx + 1
    !micro_proc_rates(i,k,idx) = micro_proc_rates(i,k,idx) + qrcol(1) ! collection rain mass by ice [kg/kg/s]
    idx = idx + 1
...
!SAM additions begin
    ...
    if(MINVAL(mtend(:,:,iqv)).gt.1.e-6) then
      ...
      mtend(:,:,iqv) = (qv(:,:)+qc(:,:)-mtend(:,:,iqv))/dt - stend(:,:,iqv) ! total water = qv + qcl
      mtend(:,:,incl) = (nc(:,:)-mtend(:,:,incl))/dt - stend(:,:,incl)
      mtend(:,:,iqr) = (qr(:,:)-mtend(:,:,iqr))/dt - stend(:,:,iqr)
      mtend(:,:,inr) = (nr(:,:)-mtend(:,:,inr))/dt - stend(:,:,inr)
      mtend(:,:,iqit) = (qitot(:,:,1)-mtend(:,:,iqit))/dt  - stend(:,:,iqit) ! assume nCat==1
      mtend(:,:,iqir) = (qirim(:,:,1)-mtend(:,:,iqir))/dt - stend(:,:,iqir)
      mtend(:,:,iqib) = (qitot(:,:,1)-mtend(:,:,iqib))/dt - stend(:,:,iqib)
      mtend(:,:,inci) = (nitot(:,:,1)-mtend(:,:,inci))/dt - stend(:,:,inci)
    end if
!SAM additions end
!.....................................................
