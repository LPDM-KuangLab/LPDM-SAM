...
!!$       dumM2 = aer_n2*RHOA*pi/6.*(aer_rm2**3)*EXP(9.*(log(aer_sig1)**2)/2.)
       ...
       case(1)
         ...
         do k = 1,kinv
           ...
           dry_ratio = MAX(0., aer_n1 - micro_field(:,:,k,incl)) / aer_n1
           micro_field(:,:,k,inad) = aer_n1*dry_ratio
           micro_field(:,:,k,iqad) = dumM1*massfactor * dry_ratio
           micro_field(:,:,k,iqaw) = dumM1*massfactor * (1. - dry_ratio)
         end do
         ...
         do k = kinv+1,nzm
           ...
           ! dry_ratio is the fraction of total aerosols that are not cloud droplets
           dry_ratio = MAX(0., aer_n2 - micro_field(:,:,k,incl)) / aer_n2
           micro_field(:,:,k,inad) = aer_n2*dry_ratio
           micro_field(:,:,k,iqad) = dumM2*massfactor * dry_ratio
           micro_field(:,:,k,iqaw) = dumM2*massfactor * (1. - dry_ratio)
         end do
